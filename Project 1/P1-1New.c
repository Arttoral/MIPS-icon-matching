...
int main(int argc, char *argv[]) {
  ...
  /* int candidateIndex, patternIndex = 0;
  Match = -1;
  while(candidateIndex / IconSize < NumCandidates && Match == -1) {
    while (patternIndex < IconSize && Pattern[patternIndex] == Candidates[candidateIndex])
    {
      patternIndex++;
      candidateIndex++;
    }

    if (patternIndex == IconSize) {
      Match = (candidateIndex / IconSize) - 1;
    } else if (patternIndex < IconSize){
      candidateIndex += (IconSize - patternIndex);
      patternIndex = 0;
    }
 ...
 int patternIndex = 0;
 int cand = 0;
  while (patternIndex < 144) {
    if (Pattern[patternIndex] != 0) {
      if (Pattern[patternIndex] != Candidates[patternIndex + (IconSize * cand)]) {
        patternIndex = -1;
        cand++;
       }
    }
